...
describe('decideTarget', () => {
  ...
  test('Pages served from the legacy gh-pages branch falls back to artifact and says how to switch', async () => {
    const octokit = octokitWithPages(jest.fn().mockResolvedValue({
      data: { build_type: 'legacy', source: { branch: 'gh-pages', path: '/' } },
    }));
    expect(await decideTarget(octokit, 'o', 'r', false)).toBe('artifact');
    expect(core.warning).toHaveBeenCalledWith(expect.stringContaining('Source to "GitHub Actions"'));
  });

  test('Pages served from another branch falls back to artifact without the switch hint', async () => {
    const octokit = octokitWithPages(jest.fn().mockResolvedValue({
