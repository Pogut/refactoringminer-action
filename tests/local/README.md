...
- **PR path with `ENABLE_WEB_VIEW=true`:** `export.js` will call the real
  ...
  which also needs a real Actions runtime to succeed). The actual GitHub
  Pages deploy (`actions/upload-pages-artifact` + `actions/deploy-pages`)
  only runs as real composite-action steps in CI — this harness can't
  exercise it, since it needs the workflow's OIDC token.
